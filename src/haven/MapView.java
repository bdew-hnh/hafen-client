...
public class MapView extends PView implements DTarget, Console.Directory {
	...
	public void updateGridOutline() {
		...
		Coord tc = cc.floor(MCache.tilesz);
	...
	public Click(Coord c, int b, boolean nomod) {
	    super(c);
	    this.clickb = b;
		this.nomod = nomod;
	}
	...
	protected void hit(Coord pc, Coord2d mc, ClickInfo inf) {
	    ...
	    if(inf == null) {
		wdgmsg("click", pc, mc.floor(posres), clickb, mod);
	    } else {
	...
	public void startMouseWalking(boolean now) {
		if (walker == null && grab == null) {
			walker = new MouseWalker(this, now);
			grab(walker);
		}
	}

	public void stopMouseWalking() {
		if (walker!=null) {
			walker.remove();
			if (walker == grab)
				release(walker);
